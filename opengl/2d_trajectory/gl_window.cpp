...
void WindowGL::initializeGL() {
  ...
  path_buffer = std::make_unique<Buffer<Vector2f, GL_ARRAY_BUFFER>>(GL_DYNAMIC_DRAW, 100000);
  path_buffer->append(std::vector<Vector2f>({ {0.0f, 0.0f} }));
...
void WindowGL::paintGL() {
  ...
  }
  {
    ...
    program->setUniform("color", Vector3f(0.0f, 1.0f, 0.0f));
    program->setAttribute("vertex", *path_buffer);
    program->enableVertexAttribArray("vertex");
    glDrawArrays(GL_LINE_STRIP, 0, path_buffer->size());
    program->disableVertexAttribArray("vertex");
  }
}
...
void WindowGL::updateMatrix(const Vector3f& pos) {
  ...
  if (t.x() != matrix(0, 2) || t.y() != matrix(1, 2)) {
    matrix(0, 2) = t.x();
    matrix(1, 2) = t.y();
    path_buffer->append(std::vector<Vector2f>({ {t.x(), t.y()} }));
  }
}
